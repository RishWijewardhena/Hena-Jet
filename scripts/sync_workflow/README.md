...
One limitation is worth stating plainly. A bootstrap standard deviation measures precision, not accuracy, so it cannot detect a biased calibration on its own. The original `outputs/test_radius_x100` result passes this gate at 0.220 mm yet sits 0.7 mm away from both later runs, with a non-overlapping confidence interval; its bias came from sparse angular coverage, 13 angles at a 30 degree step with two contributing no poses. The defences are a fine angular step and running the calibration twice and comparing, which is the only way the bias becomes visible.
