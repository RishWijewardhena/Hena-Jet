...
A calibration is only accepted when its own circle fit is good. Reprojection error reports how cleanly the marker corners were detected and says nothing about whether the fitted circle describes the machine, so `test_radius.py` gates the fit separately: `--max-fit-rmse-mm` (2 mm) and `--max-fit-residual-mm` (5 mm) apply to the inlier residuals, and `--min-ippe-error-ratio` (2.0) rejects an ambiguous single-marker pose.

Reprojection error is gated per marker count, because it does not mean the same thing on both solver paths. A single marker gives four coplanar points and IPPE solves those almost exactly, so the error is near zero however wrong the pose is. In `outputs/radius_x100_rerun` the 145 single-marker poses had a median error of 0.124 px, 48 of them below 0.10 px, while the 90 better-conditioned two-marker poses had a median of 1.050 px. The former shared 1.5 px limit therefore admitted every single-marker pose and rejected 22 poses -- **all of them two-marker**, the only measurements the number could actually judge. `--max-reprojection-error-px` (3.0) now applies to multi-marker poses and `--max-single-marker-reprojection-error-px` (1.0) to single-marker ones, where it is a sanity check rather than a quality gate.

`build_fit_samples` also prefers multi-marker frames: an angle that saw two markers in any accepted frame contributes only those. Pass `--no-prefer-multi-marker` to disable it. On the recorded run this changes little, because angles that saw two markers saw them in nearly every frame, but it protects a mixed angle from having its weaker poses averaged in.

Every solved pose records its camera centre whether or not it was accepted, so a threshold can be re-evaluated against a recorded run instead of re-scanning the rig.

Requiring two markers is not an option on this rig. The four-face profile shows a second marker only near corner-on views: in `outputs/test_radius_x100`, only 39 of 107 accepted poses saw two markers, and only 4 of 13 angles kept six such poses, which fails the angular-coverage gate outright. So `--min-markers-per-pose` stays at 1 and the ambiguity is attacked directly instead. A square marker admits two IPPE poses mirrored about the image plane, and when both survive the cheirality and outward-normal tests, picking the lower reprojection error is a coin flip between poses that differ mostly *out of plane* -- the one direction the orbit fit depends on. The solver now reports `ippe_error_ratio`, how much better the winning solution fit than the discarded one, and a pose is rejected unless the winner is at least `--min-ippe-error-ratio` times better. `outputs/test_radius_x100/radius_calibration.json` predates these gates: it is stamped `valid` with a 6.370 mm inlier RMSE and a 19.188 mm maximum inlier residual, and it fails them.
