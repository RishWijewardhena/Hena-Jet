...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Clear existing PNG frames and dataset metadata in --out-dir before "
            "capturing. Use this when replacing a previous dataset."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="NEURAL" if "NEURAL" in DEPTH_MODES else "QUALITY",
    )
...
def clear_existing_dataset(out_dir: Path) -> None:
    for subdir_name in ("image", "depth"):
    ...
    for filename in ("intrinsic.json", "capture_config.json"):
...
def main() -> None:
    ...
    if args.overwrite:
        clear_existing_dataset(args.out_dir)
